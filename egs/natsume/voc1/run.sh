...
if [ "${stage}" -le -1 ] && [ "${stop_stage}" -ge -1 ]; then
    ...
    if [ ! -e "${download_dir}/Natsume_Singing_DB" ]; then
    	echo "ERROR: Natsume_Singing_DB data does not exist."
    	echo "ERROR: Please download https://bowlroll.net/file/224647 and locate it at ${download_dir}"
    	exit 1
    fi
fi
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    python local/dataset_split.py "${download_dir}/Natsume_Singing_DB" \
    ...
    for x in "${train_set}" "${dev_set}" "${eval_set}"; do
        ...
        awk < "${src_data}/segments" '{printf("%s natsume\n", $1);}' > ${src_data}/utt2spk
    done
fi
...
if [ -z "${tag}" ]; then
    expdir="exp/${train_set}_natsume_$(basename "${conf}" .yaml)"
else
    expdir="exp/${train_set}_natsume_${tag}"
fi
