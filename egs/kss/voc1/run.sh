...
conf=conf/melgan.v3.yaml

# directory path setting
download_dir=downloads/archive # direcotry to save downloaded files
...
if [ -z "${tag}" ]; then
    expdir="exp/${train_set}_kss_$(basename "${conf}" .yaml)"
else
    expdir="exp/${train_set}_kss_${tag}"
fi
