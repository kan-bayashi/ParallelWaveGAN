...
def _get_basename(path: str):
    # logging.info(f'basename:{os.path.splitext(os.path.split(path)[-1])[0]}')
    return os.path.splitext(os.path.split(path)[-1])[0].split('_')[0]
...
def calculate(
    file_list: List[str],
    gt_file_list: AudioDataset,
    args: argparse.Namespace,
    mcd_dict: Dict,
):
    """Calculate MCD."""
    # print(f'gt_file_list:{gt_file_list.utt_ids}')
    for i, gen_path in enumerate(file_list):
        gt_basename = gen_path.split('/')[-1].split('.')[0][:-4]
        corresponding_list = list(
            filter(lambda i: gt_file_list.utt_ids[i] == gt_basename in gen_path, range(len(gt_file_list.utt_ids)))
        )
        # corresponding_list = [ i for i in range(len(gt_file_list.utt_ids)) if gt_file_list.utt_ids[i] == gt_basename ]
        # logging.info(f'gen_path:{gt_basename}')
        # logging.info(f'corresponding_list:{corresponding_list}')
        # logging.info(f'gt_file_list:{gt_file_list.utt_ids}')
        ...
        gen_x, gen_fs = sf.read(gen_path)
        gen_x = gen_x.astype(np.float32)
        gen_x /= 1 << (16 - 1)
        gt_x, gt_fs = gt_file_list[gt_path]
...
def main():
    ...
    if os.path.isdir(args.gt_wavdir_or_wavscp):
        ...
        if gt_files[0].endswith("|"):
            try:
                gt_files = AudioSCPDataset(
                    args.gt_wavdir_or_wavscp,
                    segments=None,
                    return_utt_id=False,
