...
dataset_check_functions = {
    DatasetType.TRAIN: train_check,
    DatasetType.DEV: dev_check,
    DatasetType.EVAL: test_check
}
...
def process_subset(args, set_name, dataset_type):
    ...
    assert dataset_type in dataset_check_functions
    check_func = dataset_check_functions[dataset_type]
    for label_file in os.listdir(args.lab_srcdir):
        if not label_file.endswith(".lab"):
            continue

        if not check_func(label_file):
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_set", type=str, help="name of training data directory")
    parser.add_argument("--dev_set", type=str, help="name of development data direcotry")
    parser.add_argument("--eval_set", type=str, help="name of evaluation data direcotry")
    args = parser.parse_args()

    for name, dataset_type in zip(
        [args.train_set, args.dev_set, args.eval_set],
        [DatasetType.TRAIN, DatasetType.DEV, DatasetType.EVAL],
    ):
        process_subset(args, name, dataset_type)