...
download_dir=/data8/tyx/dataset # set the directory to your database
...
if [ "${stage}" -le -1 ] && [ "${stop_stage}" -ge -1 ]; then
    ...
    if [ ! -e "${download_dir}/OFUTON_P_UTAGOE_DB" ]; then
    	echo "ERROR: OFUTON_P_UTAGOE_DB data does not exist."
    	echo "ERROR: Please download https://sites.google.com/view/oftn-utagoedb/%E3%83%9B%E3%83%BC%E3%83%A0 "
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    python local/dataset_split.py "${download_dir}/OFUTON_P_UTAGOE_DB" \
    ...
    for x in "${train_set}" "${dev_set}" "${eval_set}"; do
        ...
        awk < "${src_data}/segments" '{printf("%s ofuton\n", $1);}' > ${src_data}/utt2spk
    done
fi
...
if [ -z "${tag}" ]; then
    expdir="exp/${train_set}_ofuton_$(basename "${conf}" .yaml)"
else
    expdir="exp/${train_set}_ofuton_${tag}"
fi
