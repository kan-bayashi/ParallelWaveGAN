...
download_dir=../../../../data/ # set the directory to your database
...
if [ "${stage}" -le -1 ] && [ "${stop_stage}" -ge -1 ]; then
    ...
    if [ ! -e "${download_dir}/no7singing" ]; then
    	echo "ERROR: no7singing data does not exist."
    	echo "ERROR: Please download"
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    python local/dataset_split.py "${download_dir}/no7singing" \
    ...
    for x in "${train_set}" "${dev_set}" "${eval_set}"; do
        ...
        awk < "${src_data}/segments" '{printf("%s no7singing\n", $1);}' > ${src_data}/utt2spk
    done
fi
...
if [ -z "${tag}" ]; then
    expdir="exp/${train_set}_no7singing_$(basename "${conf}" .yaml)"
else
    expdir="exp/${train_set}_no7singing_${tag}"
fi
